import { Config, ConfigParameters, getConfig, AuthorizationParameters, DeepPartial } from '../../src/zauth-session';
...
describe('Config', () => {
  ...
  it('should set idpLogout to true when zAuthLogout is true', () => {
    const config = getConfig({
      ...defaultConfig,
      zAuthLogout: true
    });
  ...
  it('zAuthLogout and idpLogout should default to false', () => {
    const config = getConfig(defaultConfig);
    expect(config).toMatchObject({
      zAuthLogout: false,
      idpLogout: false
  ...
  it('should not set zAuthLogout to true when idpLogout is true', () => {
    const config = getConfig({
      ...defaultConfig,
      idpLogout: true
    });
