...
import { getConfig, clientFactory, ConfigParameters } from '../../src/zauth-session';
...
const getClient = (params: ConfigParameters = {}): Promise<Client> =>
  clientFactory(getConfig({ ...defaultConfig, ...params }), { name: 'nextjs-zauth', version })();
...
describe('clientFactory', function () {
  ...
  it('should send the correct default headers', async function () {
    ...
    expect(headerProps).toContain('zauth-client');

    const decodedTelemetry = JSON.parse(Buffer.from(headers['zauth-client'] as string, 'base64').toString('ascii'));

    expect(decodedTelemetry.name).toEqual('nextjs-zauth');
    ...
    expect(headers['user-agent']).toEqual(`nextjs-zauth/${pkg.version}`);
  });
  ...
  it('should disable telemetry', async function () {
    ...
    expect(headerProps).not.toContain('zauth-client');
  });
